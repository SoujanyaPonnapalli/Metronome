...
DATA_OUT     = DATA_DIR / "per-cell-parsed.csv"
PROGRESS_LOG = PROJECT_ROOT / "E19-progress.jsonl"
RUN_LOG      = PROJECT_ROOT / "E19-run.log"
...
NS    = [3, 5, 7]
MODES = ["vanilla", "metronome_kf1"]   # terraform-side tags
AZS   = ["intraAZ", "crossAZ"]
...
class CellSpec:
    ...
    def base_id(self) -> str:
        return f"E19-n{self.n}-v{self.val}-{self.az}-{self.mode}-c{self.c}"
...
def handle_cell(cell: CellSpec, infra: dict, wid: int,
    ...
    log.info(f"[w{wid}] start {cell.base_id}")
    runs: list[RunResult] = []
