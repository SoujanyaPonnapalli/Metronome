...
type Config struct {
	...
	// Metronome, when non-nil, enables the metronome per-entry
	// durability-gated commit rule. Under it the leader commits index j only
	// when at least Metronome.Quorum() distinct replicas have j DURABLY on
	// disk, determined from each replica's durable watermark (Progress.Durable
	// for followers, Metronome.SelfDurable() for the leader) against the
	// deterministic persist-set Metronome.ShouldPersist. Nil preserves the
	// stock median-of-Match commit. See metronome.go.
...
type raft struct {
	...
	// metronome, when non-nil, replaces the median-of-Match commit rule with
	// the per-entry durability-gated rule (see metronome.go).
	metronome MetronomeOracle
	// Leader-driven recovery (raft goroutine, tick-based; works even when the
	// application's Ready loop is blocked behind a stalled commit):
	//   metronomeTick          - absolute leader tick counter
	//   metronomeLastCommit    - committed index observed at the last tick
	//   metronomeStalledTicks  - consecutive ticks the per-entry commit has
	//                            been stuck with entries pending
	//   metronomeFallbackUntil - tick until which commit uses the standard
	//                            median-of-acks rule (persist-everything window)
	metronomeTick          uint64
	metronomeLastCommit    uint64
	metronomeStalledTicks  int
	metronomeFallbackUntil uint64
	// metronomeVD is a reusable scratch map (voter id -> validated durable
	// watermark) for metronomeCommitted. Reused and cleared in place every
	// call to avoid a heap allocation on the commit hot path, which runs on
	// every MsgAppResp.
	metronomeVD map[uint64]uint64
}
...
func (r *raft) maybeCommit() bool {
	...
	// Under metronome, the commit index is gated by per-entry durability
	// (see metronome.go) instead of the median of Match. The candidate still
	// flows through raftLog.maybeCommit, preserving raft's current-term gate.
	//
	// EXCEPTION: while this node is in work-steal "log everything" mode it is
	// fully replicating (no persist-set holes) to recover from a straggler, so
	// the per-entry rule — which only counts persist-set members — would
	// deadlock on indices whose persist-set includes the down node. Fall back
	// to the standard median-of-acks commit, which is correct here because the
	// logging nodes hold every entry durably.
	idx := r.trk.Committed()
	// Use the per-entry durability rule only in steady state. Fall back to the
	// standard median-of-acks commit while recovering from a straggler — either
	// because this node is in work-steal "persist-everything" mode
	// (SelfLogEverything), or because the leader-driven stall detector opened a
	// fallback window (metronomeFallbackUntil; see tickHeartbeat). During
	// recovery the surviving nodes are fully replicating, so the standard rule
	// is correct; the per-entry rule — which credits only persist-set members —
	// would otherwise wedge commit on indices whose persist-set includes the
	// down node.
	if r.metronome != nil && !r.metronome.SelfLogEverything() && r.metronomeTick >= r.metronomeFallbackUntil {
		idx = r.metronomeCommitted()
	}
	return r.raftLog.maybeCommit(entryID{term: r.Term, index: idx})
}
...
func (r *raft) tickHeartbeat() {
	...
	// Metronome leader-driven recovery: if the per-entry commit rule leaves
	// commit stuck (with entries pending) for longer than the work-steal
	// timeout, enter a standard-commit fallback window so a node-down failure
	// can't wedge commit. This runs in the raft goroutine and so works even
	// when the application's Ready loop is blocked behind the stalled commit.
	if r.metronome != nil {
		r.metronomeTick++
		switch {
		case r.raftLog.committed != r.metronomeLastCommit:
			r.metronomeLastCommit = r.raftLog.committed
			r.metronomeStalledTicks = 0
		case r.raftLog.lastIndex() > r.raftLog.committed:
			r.metronomeStalledTicks++
			if st := r.metronome.StallTicks(); st > 0 && r.metronomeStalledTicks >= st {
				r.metronomeFallbackUntil = r.metronomeTick + uint64(r.metronome.WindowTicks())
				r.metronomeStalledTicks = 0
				if r.maybeCommit() { // unblock now; don't wait for the next message
					r.bcastAppend()
				}
			}
		}
...
func stepLeader(r *raft, m pb.Message) error {
	...
	switch m.Type {
	case pb.MsgBeat:
		// Metronome: re-evaluate commit on each heartbeat. The leader's own
		// durable watermark and work-stolen out-of-stripe copies advance in
		// the application's storage path with NO incoming raft message, so
		// nothing else triggers maybeCommit for an index the leader just
		// persisted to keep the cluster live under a failure. Without this,
		// commit can stall permanently even though the leader holds a durable
		// copy. Bounded staleness = one heartbeat interval.
		if r.metronome != nil && r.maybeCommit() {
			r.bcastAppend()
		}
		r.bcastHeartbeat()
		return nil
	...
	case pb.MsgAppResp:
		...
		if m.Reject {
			...
			// Metronome: fold in the follower's durable watermark / stolen
			// report (carried in Context). durAdvanced is true when the
			// durability count changed without Match advancing — a straggler
			// catching up on persistence — so we still recompute commit below.
			durAdvanced := r.ingestDurableReport(m.From, m.Context)
			...
			if pr.MaybeUpdate(m.Index) || (pr.Match == m.Index && pr.State == tracker.StateProbe) {
				...
				// Metronome: the durable count advanced without Match moving
				// (a straggler caught up on persistence) — recompute commit.
				if r.maybeCommit() {
					releasePendingReadIndexMessages(r)
					r.bcastAppend()
				}
