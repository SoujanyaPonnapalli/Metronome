...
type Progress struct {
	...
	// Durable is the metronome durable watermark reported by the follower:
	// the highest index up to which every entry this node was assigned to
	// persist (its persist-set "stripe") is on its disk, with no gap. It is
	// used ONLY by the metronome per-entry durability commit rule and does
	// NOT drive flow control, leadership transfer, or snapshot decisions —
	// those continue to use Match (the in-memory ack). Zero when metronome
	// is disabled. DurableTerm is the term the follower reported for Durable,
	// so the leader can ignore a watermark that no longer matches its log.
	Durable     uint64
	DurableTerm uint64
...
// MaybeUpdateDurable advances the metronome durable watermark (and its term)
// reported by the follower. It is monotone in the index and, unlike
// MaybeUpdate, touches NOTHING else — not Match, Next, flow-control, or
// inflights. Returns false if the report is stale (index not ahead). Used
// only by the metronome per-entry durability commit rule.
func (pr *Progress) MaybeUpdateDurable(n, term uint64) bool {
	if n <= pr.Durable {
		return false
	}
	pr.Durable = n
